...
import { ChevronLeft, ChevronRight, Download, Save, Trash } from 'lucide-react';
      ...
      >
        <div 
          className={`absolute inset-0 ${isTransitioning ? '' : 'transition-all duration-500'} ${
            flipped ? 'opacity-0 -rotate-y-180' : 'opacity-100 rotate-0'
          }`}
        >
        ...
        <div 
          className={`absolute inset-0 ${isTransitioning ? '' : 'transition-all duration-500'} ${
            flipped ? 'opacity-100 rotate-0' : 'opacity-0 rotate-y-180'
          }`}
        >
