...
const AnkiCards: React.FC<AnkiCardsProps> = ({ ankiCards }) => {
  ...
  const handleExport = () => {
    // Generate content in Anki import format
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center">
          ...
          <span className="ml-2 px-2 py-1 bg-violet-100 text-violet-800 text-xs rounded font-medium">
            Premium
          </span>
        </div>
      ...
      <div className="space-y-4">
        {ankiCards.map((card, index) => (
          ...
          >
            <CardHeader className="flex flex-row justify-between items-center cursor-pointer" onClick={() => setExpandedCardIndex(expandedCardIndex === index ? null : index)}>
